...
class PointCloudPostProcessor:
    ...
    def csv_to_asc(self, csv_file, las_file, asc_file):
        # Read the CSV file
        ...
        source = self.nearestToCorner(self.box_corners(las_file), np_coords) - 8
    ...
    def csvToLas(self, test_dir, test_index, length):
        ...
        for x in range(length):
            ...
            self.csv_to_asc(csv_file, las_file, asc_file)
    ...
    def create_components(self, project_id, task_id, uid, color): 
        min_p = 4
        tolerance = .65
        max_p = 10000
        ...
        index = self.create_csvs(test_dir)
        self.csvToLas(test_dir, test_index, index + 1)
...
def components_api():
    ...
    processed_data = ppp.create_components(project_id, task_id, uid, color)
